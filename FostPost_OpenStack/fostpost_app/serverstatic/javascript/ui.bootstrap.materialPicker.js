angular.module('ui.bootstrap.materialPicker', []).directive('ngBindModel',function($compile){
    return{
...
.directive('materialPicker', [
    ...
    function ($parse) {
        ...
        return {
            restrict: "E",
            require: 'ngBindModel',
            link: function ($scope, element, attrs, ngBindModel) {
                ...
                var action = function (color, apply) {
                    ...
                    if (format() === 'hex') {
                        ngBindModel.$setViewValue('#' + color.hex);
                    } else {
                        var rgb = hexToRgb(color.hex);
                        ngBindModel.$setViewValue({
                            hex: '#' + color.hex,
                ...
                $scope[watch](attrs.ngBindModel, function (value, oldValue) {
                    ...
                    if (isHex(value.hex || value)) {
                        ngBindModel.$setValidity('required', true);
                    } else {
                        ngBindModel.$setValidity('required', false);
                    }
